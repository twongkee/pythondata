import pandas as pd
from twutils import getconfig, setuplogging
...
df_meta_min=df_meta.drop(columns=['Nasdaq Traded', 'Listing Exchange','ETF','Market Category','Round Lot Size', 'Test Issue','Financial Status','CQS Symbol','NextShares'])
joined_df_list=list()
for df in df_all:
    df_joined=df.join(df_meta_min.set_index('NASDAQ Symbol'), on='NASDAQ Symbol', how='left')
...
for df in joined_df_list:
    ...
    df_features=df.drop(columns=['Open','High','Low','Close','Adj Close','Security Name','Symbol'])
